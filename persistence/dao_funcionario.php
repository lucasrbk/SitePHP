<?php
	...
	class DAOFuncionario{
		...
		public function cadastrarFuncionario(Funcionario $funcionario, $codigoCargo){
			...
			$sql = "INSERT INTO funcionario (nome, cpf, dataadmissao, status, cargo) 
						 VALUES ('$nome', '$cpf', '$dataAdmissao', '$status', '$codigoCargo')";
			$this->conexao->executarQuery($sql);
		}
		
		public function listarFuncionarios(){
			$lista = $this->conexao->executarQuery("SELECT f.codigo as funcionario, f.nome as nomefuncionario, f.cpf, f.dataadmissao, f.status, 
														c.codigo as cargo, c.nome as nomecargo, c.descricao, c.salariobase
 														FROM funcionario f, cargo c WHERE f.cargo = c.codigo");
			$arrayFuncionarios = array();
			
			while ($linha = mysqli_fetch_array($lista)) {
				$cargo = new Cargo($linha['cargo'], $linha['nomecargo'], $linha['descricao'], $linha['salariobase']);
				$funcionario = new Funcionario($linha['funcionario'],$linha['nomefuncionario'],$linha['cpf'],
